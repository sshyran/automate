...
func init() {
  ...
  "definitions": {
    ...
    "chef.automate.api.compliance.reporting.v1.DataToExport": {
      "type": "object",
      "properties": {
        "blacklist_fields": {
          "type": "array",
          ...
          "description": "Fields to skip (NOT include) in the exported results. Not allowed to combine with ` + "`" + `whitelist fields` + "`" + `."
        },
        "whitelist_fields": {
          "type": "array",
          "items": {
          ...
          "description": "Fields to include in the exported results."
        }
      },
      "description": "Used to specify which fields should be included in the report. Only ` + "`" + `blacklist_fields` + "`" + ` OR ` + "`" + `whitelist_fields` + "`" + ` is allowed. \nCombining the two will result in an error."
    },
    ...
    "chef.automate.api.compliance.reporting.v1.Query": {
      ...
      "properties": {
        ...
        "data_to_export": {
          "$ref": "#/definitions/chef.automate.api.compliance.reporting.v1.DataToExport",
          "description": "Intentionally blank."
        }
      }
    },
